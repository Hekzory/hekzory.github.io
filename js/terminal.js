...
// intent is clearer — we're populating newly-created elements, not replacing
// arbitrary DOM. All HTML strings in this module are built from trusted sources
// (hardcoded scenarios, SYS config, escape-then-wrap syntax highlighting).
...
class Terminal {
    ...
    createContext() {
        return {
            ...
            cwdString: () => this.currentPath.join('/'),
    ...
    async typeCommand(text, signal) {
        ...
        injectHTML(line, this.generatePromptHTML(true, true));
        ...
        injectHTML(cmdSpan, this.highlightCommand(currentText));
        await this.wait(150);
    ...
    printPrompt() {
        const line = document.createElement('div');
        line.className = 'line posh-prompt-block';
        injectHTML(line, this.generatePromptHTML(true, false));
        this.element.appendChild(line);
        this.scrollToBottom();
    }
    ...
    addOutput(text) {
        ...
        for (const lineText of lines) {
            ...
            injectHTML(div, this.highlightSyntax(lineText));
